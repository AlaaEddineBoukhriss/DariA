...
public class ReclamationController {
    ...
    @PostMapping("/add/{id}")
    public Reclamation AddReclamationAnnonce(@RequestBody Reclamation r, @PathVariable Long id)   
    {   long l=1;
    	Annonce a = service.retrieveAnnonce(id);
    	r.setAnnonce(a);
        User user= service.retrieveUser(l);
        r.setUser(user);
        return service.addReclamation(r); 
    	
    } 
    @PostMapping("/add")
    public Reclamation AddReclamationuser(@RequestBody Reclamation r)   
    {   long l=1;
        User user= service.retrieveUser(l);
        r.setUser(user);
        return service.addReclamation(r); 
